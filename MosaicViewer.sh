...
#run NCRF
cat $SAMPLE_NAME"_trimmed_"$SIDE"_fw.fasta" | $NCRF_DIR"/NCRF" --scoring=nanopore --minlength=15 CAG_repeat:CAG --minmratio=0.90 --stats=events --positionalevents > $SAMPLE_NAME"_"$SIDE"_CAG.ncrf"
cat $SAMPLE_NAME"_trimmed_"$SIDE"_fw.fasta" | $NCRF_DIR"/NCRF" --scoring=nanopore --minlength=15 CAA_repeat:CAA --minmratio=0.90 --stats=events --positionalevents > $SAMPLE_NAME"_"$SIDE"_CAA.ncrf"
cat $SAMPLE_NAME"_trimmed_"$SIDE"_fw.fasta" | $NCRF_DIR"/NCRF" --scoring=nanopore --minlength=15 CGG_repeat:CGG --minmratio=0.90 --stats=events --positionalevents > $SAMPLE_NAME"_"$SIDE"_CGG.ncrf"

NCRF_files=$(find $PIPELINE_DIR -maxdepth 1 | grep $SAMPLE_NAME | grep $SIDE | grep "\\.ncrf$")
$NCRF_DIR"/ncrf_cat.py" $NCRF_files | $NCRF_DIR"/ncrf_sort.py" --sortby=name | $NCRF_DIR"/ncrf_summary.py" | grep -P "^#|\+" > $SAMPLE_NAME"_trimmed_"$SIDE"_fw.ncrf.summary"
